...
class GoogleAuthView(APIView):
    ...
    def post(self,request):
        ...
        if not token or mode not in ['signin','signup']:
            return Response({"error":"Please provide a valid token and mode (signin or signup)"},status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if mode == 'signin' and not user_exists:
                return Response({"error":"User with this email does not exist"},status=status.HTTP_400_BAD_REQUEST)
            
            user, _ = User.objects.get_or_create(email=email, defaults={
                'first_name': first_name,
                'last_name': last_name,
                'is_active': True  # Automatically activate the user on signup
            })
...
class UserListView(generics.ListAPIView):
    ...
    search_fields = ['first_name','middle_name','last_name','phone_number','address','email']
    filterset_fields = {
...
def send_password_reset_email(request):
    ...
    send_mail(
        ...
        message=f"Click the link below to reset your password:\n\n{current_site}/reset-password/{token}",  # Plain text version
...
def get_user_profile(request):
    ...
    try:
        ...
        if user.profile_picture and hasattr(user.profile_picture, 'url'):
           profile_pic_url = current_site.domain+"/"+str(user.profile_picture.url)
        else:
